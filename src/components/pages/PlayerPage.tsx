...
export const PlayerPage: React.FC<Props> = ({navigation}) => {
  ...
  const selected_episode_id = useSelector((state: RootState) => state.course.selected_episode_id);
  const selected_episode_title = useSelector(
    (state: RootState) => state.course.selected_episode_title
  );
  ...
  async function playSound() {
    ...
    const {sound} = await Audio.Sound.createAsync(
      require(`../../assets/audio/sess_0${selected_episode_id || 0}.m4a`)
    );

    sound.setOnPlaybackStatusUpdate(status => console.log(setPosition(status['positionMillis'])));
    setSound(sound);
  ...
  return (
    <ImageBackground source={bg_image} blurRadius={20} style={styles.page_container}>
      ...
      >
        <TouchableOpacity onPress={() => sound1.setPositionAsync(position - 5000)}>
          <Image style={{width: 50, height: 50, resizeMode: 'contain'}} source={forward_icon} />
        </TouchableOpacity>
        ...
        <TouchableOpacity onPress={() => sound1.setPositionAsync(position + 5000)}>
          <Image style={{width: 50, height: 50, resizeMode: 'contain'}} source={backward_icon} />
        </TouchableOpacity>
      </View>
